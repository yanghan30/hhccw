...
class biz_use_coupon{
	...
	{  
		...
		if($supplier_id > 0)
		{
			...
			if ($biz_user['is_main'] == 1){
				$location_id = $GLOBALS['db']->getOne("select id as location_id from ".DB_PREFIX."supplier_location where supplier_id = ".$biz_user['supplier_id']);
			}else{
				$location_id = $GLOBALS['db']->getOne("select location_id from ".DB_PREFIX."supplier_account_location_link where account_id = ".$biz_user['id']);
			}
			$result = biz_use_coupon($biz_user,$pwd,$location_id);
