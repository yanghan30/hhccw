...
function getGoodsArray($item){
	...
	$goods['share_url']= get_domain()."/".FOLDER_NAME."/index.php?ctl=goodsdesc&id=".$item['id'];
	$goods['share_content']=emptyTag($item['name']).$goods['share_url'];
	...
	foreach($cartdata as $key=>$cart)
	{
		...
		*/
		...
		$cart_item = array();
			...
			$cart_item['is_shop'] = $deal_info['is_shop'];
			$cart_list[] = $cart_item;
		//end
	}
	if(!$score_enough)
...
function pinsertCartData($user_id,$session_id,$cartdata)
{
	$GLOBALS['user_info']['id'] = $user_id;
	require APP_ROOT_PATH.'system/model/deal.php';
	//require APP_ROOT_PATH.'app/Lib/deal.php';
...
function init_order_parm($MConfig){
	...
	*/
	...
	if ($GLOBALS['request']['from'] == 'wap'){
		...
		//支付列表
		$sql = "select id, class_name as code, name, 1 as has_calc from ".DB_PREFIX."payment where (online_pay = 2 or online_pay = 3 or `class_name` in('Malipay','Mtenpay','Mcod')) and `class_name` <> 'Wwxjspay' and is_effect = 1";
	}
...
function m_get_topic_item($topic)
{
		...
		//$share_item['share_content'] =  msubstr($topic['content']).get_domain().str_replace("sjmapi/","",url("index","topic",array("id"=>$topic['id'],"is_pc"=>"1")));	
		$share_item['share_content'] =  msubstr($topic['content']).get_domain()."/index.php?ctl=topic&id=".$topic['id']."&is_pc=1";
		$share_item['share_url'] =  get_domain()."/index.php?ctl=topic&id=".$topic['id']."&is_pc=1";
...
function get_collect_list($limit,$user_id)
	...
	function getWebAdsUrl($type,$data){
		//0:URL广告;11:团购列表;12:商品列表;14:活动列表;15:优惠列表;21:团购明细;21:商品明细;25:优惠明细;16:商家列表;26：商家明细; 31:扫一扫;32:附近会员;
		if ($type == 0){
			$url = "http://".$data['url'];
		}else if ($type == 11){
			if($data['cate_id'])
			...
			if($data['cate_id'])
			$url =get_domain().APP_ROOT."/".APP_INDEX."/index.php?ctl=merchantlist&cate_id=".$data['cate_id'];
			else 
				$url =get_domain().APP_ROOT."/".APP_INDEX."/index.php?ctl=merchantlist";
		}else if ($type == 26){
	...
	function getCateList(){
		$bigcate_list=$GLOBALS['db']->getAll("select id,name from ".DB_PREFIX."deal_cate where is_delete=0 and is_effect=1 and pid=0 order by sort asc,id desc");
		$bcate_list=array();
		$bcate_ids=array();
		$bcate_type=array();
	...
	$allcate_list=$GLOBALS['db']->getAll("select id,name,pid from ".DB_PREFIX."shop_cate where is_delete=0 and is_effect=1");
		$cate_list=array();
		$bcate_list=array();
