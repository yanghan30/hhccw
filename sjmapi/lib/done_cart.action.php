	...
	public function index(){
		...
		if($user_id>0)
		{
			...
			if($GLOBALS['request']['from']=="wap"){
				...
				$res = insertCartData($user_id,es_session::id(),$my_array);	
			}else{
				$cartdata = $GLOBALS['request']['cartdata'];
				$len=count($cartdata,0);
				for($i=0;$i<$len; $i++) {
					for($j=$i+1;$j<$len;$j++) {
						if($cartdata[$i]['goods_id'] ==$cartdata[$j]['goods_id']){
							$cartdata[$i]['num']+=$cartdata[$j]['num'];
						}
					}
				}
			...
			else
			{
				...
				else
				{
					...
					foreach($goods_list as $k=>$v)
					{
						...
						$goods_item['supplier_id']	=	$v['supplier_id'];
						...
						$goods_item['delivery_status'] = $data['is_delivery']==0?5:0;
