...
class mobile_qrcode{
	...
	public function decode_goods($pc_url)
	{
		...
		if (strpos($pc_url, 'ctl=deal') || strpos($pc_url, '/deal/')){
			...
			$type = 10;
		}else if(strpos($pc_url, 'ctl=mall') || strpos($pc_url, '/mall')){
			$type = 10;
		}else{
			return false;
		}
	...
	{//http://www.sh591.cn/index.php?ctl=deal&act=159
		...
		//}else{
			...
			}else if(strpos($pc_url, 'ctl=tuan') || strpos($pc_url, '/tuan')){
				$type = 9;
			}else{
				//$id = $this->get_id($pc_url,'act');
				//$type = 9;	
	...
	public function decode_yh($pc_url)
	{
		...
		//}else{
			if (strpos($pc_url, '/stores') || strpos($pc_url, 'ctl=stores')){				
				...
				//$id = $this->get_id($pc_url,'act');
				$type = 11;				
			}else if (strpos($pc_url, '/youhuis')|| strpos($pc_url, 'ctl=youhuis')){				
				//&id=19 || id-19				
				//$id = $this->get_id($pc_url,'act');
				$type = 12;				
			}else if (strpos($pc_url, '/youhui')|| strpos($pc_url, 'ctl=youhui')){				
				//&id=19 || id-19				
				//$id = $this->get_id($pc_url,'act');
	...
	function get_id($pc_url,$key)
	{
		if (strpos($pc_url, $key)){
			if (strpos($pc_url, '?')){
				//无重写
				$key2 = '&'.$key."=";
				if (strpos($pc_url,$key2)){
					$start = strpos($pc_url,$key2) + strlen($key2);	
				}else{
					$key2 = '?'.$key."=";
					$start = strpos($pc_url,$key2) + strlen($key2);
				}
				$str = substr($pc_url, $start, strlen($pc_url) - $start);
				if (strpos($str, '&')){
