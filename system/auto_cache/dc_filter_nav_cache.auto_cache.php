<?php
//团购模块的筛选切换单菜
class dc_filter_nav_cache_auto_cache extends auto_cache{
	public function load($param)
	{
		...
		if($result===false||IS_DEBUG)
		{
			...
			$bquan_list[] = array("url"=>url("index","dclocation",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
			...
			foreach($bquan_list_res as $k=>$v)
			{
					...
					unset($tmp_url_param['qid']);
					$durl = url("index","dclocation",$tmp_url_param);
					$v['url']=$durl;
			...
			if($deal_area_id>0)
			{
					...
					if($squan_list_res)
					{
						...
						$tmp_url_param = $url_param;
						unset($tmp_url_param['qid']);
						$squan_list[] = array("url"=>url("index","dclocation",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
					}
			...
			$bcate_list[] = array("url"=>url("index","dclocation",$tmp_url_param),"name"=>$GLOBALS['lang']['ALL'],"current"=>$all_current);
			...
			{		
						...
						$v['url'] = url("index","dclocation",$tmp_url_param);
